/**
 * AdministratorEnemy - Custom Night Enemy
 *
 * The Administrator controls the mercenaries' contracts and infrastructure from
 * behind the scenes. She never enters the base directly — she works remotely,
 * hacking teleporter nodes to cut off Engineer's escape routes and mobility.
 *
 * LORE: The Administrator may have orchestrated the infection itself. Engineer is
...
export type AdministratorState = 'WAITING' | 'TARGETING' | 'HACKING' | 'SCARED' | 'INACTIVE';

export class AdministratorEnemy {
  private state: AdministratorState = 'INACTIVE';
  ...
  /**
   * Activate Administrator (called when Custom Night starts with her enabled)
   */
  public activate(): void {
    this.state = 'WAITING';
    console.log('📋 Administrator activated — watching the base remotely');
  }
    ...
    switch (this.state) {
      ...
      case 'TARGETING':
        ...
        if (this.warnTimer >= this.warnDuration) {
          this.state = 'HACKING';
          this.hackTimer = 0;
          console.log(`📋 Administrator began hacking ${this.targetNode}!`);
        }
        break;

      case 'HACKING':
        this.hackTimer += delta;
        if (this.hackTimer >= GAME_CONSTANTS.ADMINISTRATOR_HACK_DURATION) {
          result.hackComplete = true;
          result.targetNode = this.targetNode;
          console.log(`📋 Administrator hacked ${this.targetNode}! Teleporter offline.`);
      ...
      case 'SCARED':
        ...
        if (this.scaredTimer >= GAME_CONSTANTS.ADMINISTRATOR_SCARE_COOLDOWN) {
          this.state = 'WAITING';
          console.log('📋 Administrator recovered — waiting for next opportunity');
        }
        break;
   ...
   * Called by GameScene when the player hasn't teleported for ADMINISTRATOR_NO_TELEPORT_THRESHOLD ms.
  ...
  public triggerFallbackHack(alreadyHacked: NodeId[] = [], excludeNode?: NodeId): void {
    ...
    const candidates = AdministratorEnemy.VALID_ROOMS.filter(r => {
    ...
    console.log(`📋 Administrator (Mode 2) targeting ${this.targetNode} — hack begins in ${(this.warnDuration / 1000).toFixed(1)}s`);
  }

  /**
   * Called when the player teleports to the Administrator's target room during TARGETING phase (Mode 2).
   */
  public scareAdministrator(): void {
    if (this.state !== 'TARGETING') return;
    console.log(`📋 Administrator scared off from ${this.targetNode}! She'll try elsewhere.`);
    this.targetNode = null;
    this.state = 'SCARED';
    this.scaredTimer = 0;
  }
  ...
  public interruptHack(): void {
    ...
    console.log(`📋 Administrator's hack on ${this.targetNode} was interrupted!`);
    this.targetNode = null;
    this.state = 'WAITING';
  }

  /**
   * Permanently deactivate (for Custom Night with Administrator disabled)
   */
  public forceDespawn(): void {
    this.state = 'INACTIVE';
    this.targetNode = null;
    console.log('📋 Administrator force despawned');
  }
  ...
  public getState(): AdministratorState {
    return this.state;
  }
  ...
  public getHackProgress(): number {
    ...
    return Math.min(this.hackTimer / GAME_CONSTANTS.ADMINISTRATOR_HACK_DURATION, 1);
  }
  ...
  private pickWarnDuration(): void {
    const min = GAME_CONSTANTS.ADMINISTRATOR_WARN_DURATION_MIN;
    const max = GAME_CONSTANTS.ADMINISTRATOR_WARN_DURATION_MAX;
  ...
  public getDebugInfo(): string {
    if (this.state === 'INACTIVE') return 'Administrator: INACTIVE';
    if (this.state === 'WAITING') return 'Administrator: WAITING';
    if (this.state === 'TARGETING') return `Administrator: TARGETING ${this.targetNode} (${((this.warnDuration - this.warnTimer) / 1000).toFixed(1)}s to hack)`;
    if (this.state === 'HACKING') return `Administrator: HACKING ${this.targetNode} (${(this.getHackProgress() * 100).toFixed(0)}%)`;
    if (this.state === 'SCARED') return `Administrator: SCARED (${((GAME_CONSTANTS.ADMINISTRATOR_SCARE_COOLDOWN - this.scaredTimer) / 1000).toFixed(1)}s)`;
    return 'Administrator: UNKNOWN';
  }
}

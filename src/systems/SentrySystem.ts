...
export class SentrySystem {
  ...
  fireWrangler(): void {
    ...
    if (this.scene.wranglerCooldown > 0) {
      this.scene.showAlert('COOLING DOWN...', 'warning');
    ...
    if (this.scene.metal < 50) {
      this.scene.showAlert('NOT ENOUGH METAL TO FIRE!', 'warning');
      this.scene.audio.playDeniedSound();
      return;
    }
    ...
    if (this.scene.isPyroEnabled() && this.scene.pyro && !this.scene.pyro.isForceDespawned()) {
      ...
      if (firingAtPyro) {
        ...
        this.destroySentry('PYRO REFLECTED! SENTRY DESTROYED!');
    ...
    if (this.scene.sentry.aimedDoor === 'LEFT') {
      ...
      if (this.scene.isScoutEnabled() && this.scene.scout.currentNode === 'LEFT_HALL' && 
        ...
        if (this.scene.isMedicEnabled() && this.scene.medic && this.scene.medic.isEnemyUbered('SCOUT')) {
          this.scene.showAlert('ÜBERED! CANNOT REPEL!', 'danger');
          hitEnemy = true; // Still counts as hitting (spent metal)
        } else {
          this.scene.scout.driveAway();
          this.scene.showAlert('SCOUT REPELLED!', 'success');
      ...
      if (this.scene.isDemomanEnabled() && this.scene.demoman.getChargeDoor() === 'LEFT' &&
        ...
        if (this.scene.isMedicEnabled() && this.scene.medic && this.scene.medic.isEnemyUbered('DEMOMAN')) {
          ...
          if (this.scene.demoman.isInBodyPhase()) {
            ...
            this.scene.showAlert('REPELLED LAST SECOND! +25 METAL', 'success');
          } else {
            this.scene.showAlert('DEMOMAN REPELLED!', 'success');
          }
      ...
      if (this.scene.isSniperEnabled() && this.scene.sniper.currentNode === 'LEFT_HALL' && 
        ...
        if (fullyRepelled) {
          this.scene.showAlert('SNIPER DRIVEN AWAY!', 'success');
          this.scene.audio.playEnemyRetreatSound();
        }
      ...
      if (this.scene.isSoldierEnabled() && this.scene.soldier.currentNode === 'RIGHT_HALL' && 
        ...
        if (this.scene.isMedicEnabled() && this.scene.medic && this.scene.medic.isEnemyUbered('SOLDIER')) {
          ...
          this.scene.showAlert('SOLDIER REPELLED!', 'success');
          this.scene.audio.playEnemyRetreatSound();
          hitEnemy = true;
        }
      }
    ...
    if (!hitEnemy) {
      ...
      if (heavyBlocking) {
        this.scene.showAlert('HEAVY IGNORES SENTRY! LURE HIM!', 'warning');
      } else {
        this.scene.showAlert('FIRED! (-50 metal)', 'info');
      }
    }
  }
  ...
  buildSentry(): void {
    if (this.scene.sentry.exists) {
      this.scene.showAlert('Sentry already exists!', 'warning');
      return;
    }
    
    // Can't build when cameras are up - must lower cameras first
    if (this.scene.isCameraMode) {
      this.scene.showAlert('Lower cameras first! (TAB)', 'warning');
      return;
    }
    
    // Can't build remotely - must be in Intel room
    if (this.scene.isTeleported) {
      this.scene.showAlert('Return to Intel to build!', 'warning');
      return;
    }
    
    if (this.scene.metal < GAME_CONSTANTS.BUILD_SENTRY_COST) {
      this.scene.showAlert('Not enough metal! (100 required)', 'warning');
      return;
    }
    ...
    this.scene.showAlert('SENTRY BUILT!', 'success');
    this.scene.audio.playSentryBuildSound();
  }

  upgradeSentry(): void {
    if (!this.scene.sentry.exists) {
      this.scene.showAlert('No sentry to upgrade!', 'warning');
      return;
    }
    ...
    if (this.scene.isTeleported) {
      this.scene.showAlert('Return to Intel to upgrade!', 'warning');
      return;
    }
    
    if (this.scene.sentry.level >= 3) {
      this.scene.showAlert('Sentry already max level!', 'warning');
      return;
    }
    
    if (this.scene.sentry.hp < this.scene.sentry.maxHp) {
      this.scene.showAlert('Repair sentry to full HP first!', 'warning');
      return;
    }
    
    if (this.scene.metal < GAME_CONSTANTS.UPGRADE_SENTRY_COST) {
      this.scene.showAlert('Not enough metal! (200 required)', 'warning');
      return;
    }
    ...
    this.updateSentryVisuals();
    this.scene.updateHUD();
    this.scene.showAlert(`SENTRY UPGRADED TO L${this.scene.sentry.level}!`, 'success');
    this.scene.audio.playSentryUpgradeSound();
  }

  repairSentry(): void {
    if (!this.scene.sentry.exists) {
      this.scene.showAlert('No sentry to repair!', 'warning');
      return;
    }
    ...
    if (this.scene.isTeleported) {
      this.scene.showAlert('Return to Intel to repair!', 'warning');
      return;
    }
    
    if (this.scene.sentry.hp >= this.scene.sentry.maxHp) {
      this.scene.showAlert('Sentry at full health!', 'warning');
      return;
    }
    ...
    if (this.scene.metal < 1) {
      this.scene.showAlert('Not enough metal!', 'warning');
      return;
    }
    ...
    this.scene.updateHUD();
    this.scene.showAlert(`+${Math.floor(actualRepair)} HP (-${Math.floor(actualCost)} metal)`, 'success');
  ...
  damageSentry(amount: number): void {
    ...
    this.scene.time.delayedCall(200, () => {
      if (this.scene.sentry.exists) {
        this.scene.sentryBody.setFillStyle(0xa63c30); // Back to RED team color
      }
    });
  ...
  /**
   * Destroy the sentry. This is the ONLY place that shows the destruction
   * banner — callers pass a specific message instead of alerting themselves,
   * because the HUD has a single banner and a second showAlert would
   * overwrite the first in the same frame.
   */
  destroySentry(alertMessage: string = 'SENTRY DESTROYED!'): void {
    ...
    this.scene.showAlert(alertMessage, 'danger');
    ...
    if (this.scene.isSoldierEnabled() && this.scene.soldier.isSieging()) {
      ...
      this.scene.showAlert('⚠ SOLDIER BREACHING IN 3 SECONDS! ⚠', 'danger');
  ...
  updateSentryVisuals(): void {
    ...
    this.scene.sentryLevelText.setText(`L${this.scene.sentry.level}`);
    
    // Scale sentry based on level
    const scale = 1.05 + (this.scene.sentry.level * 0.15);
  ...
  triggerAutoDefense(enemyType: string): void {
    ...
    // Sentry fires once and destroys itself
    this.destroySentry(`AUTO-DEFENSE vs ${enemyType}! SENTRY DESTROYED!`);
  }
  ...
  handleUberedEnemyEscaped(enemyType: UberTarget): void {
    ...
    if (this.scene.sentry.exists) {
      this.destroySentry('SENTRY DESTROYED BY ÜBER!');
    }
